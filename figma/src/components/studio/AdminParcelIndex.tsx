  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        {[
          ['500', 'Candidate parcels'],
          [String(tierCount('Tier A')), 'Tier A · 2 DADU'],
          [String(tierCount('Tier B')), 'Tier B · 1 DADU'],
          ['HB 1337', 'Baseline compliance'],
        ].map(([v, l]) => (
      ...
      <div className="flex flex-wrap items-center gap-3 mb-3">
        <div className="relative flex-1 min-w-[240px]">
          ...
          <input
            ...
            placeholder="Search address, APN, or zone…"
          ...
          {(['all', 'Tier A', 'Tier B'] as const).map((t) => (
            <button
              ...
              className={`px-3 py-2 rounded-lg text-[12px] font-semibold border ${
                tier === t ? 'bg-[#2B7FFF] text-white border-[#2B7FFF]' : 'bg-white text-slate-500 border-slate-200'
              }`}
            >
              {t === 'all' ? 'All' : t}
            </button>
        ...
        <span className="text-[12px] text-slate-400 ml-auto">{rows.length} of 500</span>
      </div>
                ...
                <tr key={p.apn + p.rank} className="border-b border-slate-100 hover:bg-slate-50/60">
                  ...
                  <td className="px-3 py-2.5">
                    <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-semibold ${
                      p.tier === 'Tier A' ? 'bg-emerald-50 text-emerald-600' : 'bg-[#2B7FFF]/8 text-[#2B7FFF]'
                    }`}>{p.tier}</span>
                  </td>
                  ...
                  <td className="px-3 py-2.5">
                    <div className="flex items-center gap-2">
                      <a href={googleMapsLink(p)} target="_blank" rel="noopener noreferrer" title="Google Maps / Street View" className="text-slate-400 hover:text-[#2B7FFF]"><MapPin className="w-4 h-4" /></a>
                      <a href={kingCountyLink(p)} target="_blank" rel="noopener noreferrer" title="King County Parcel Viewer" className="text-slate-400 hover:text-[#2B7FFF]"><Building2 className="w-4 h-4" /></a>
                      <a href={seattleGisLink(p)} target="_blank" rel="noopener noreferrer" title="Seattle GIS" className="text-slate-400 hover:text-[#2B7FFF]"><ExternalLink className="w-4 h-4" /></a>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <p className="text-[11px] text-slate-400 mt-3">
        Top 500 by rank · Google Maps, King County Parcel Viewer, and Seattle GIS links open in a new tab.
      </p>
    </div>
  );
}
