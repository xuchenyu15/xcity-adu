...
// Placeholder image per (room, flooring, cabinets, furniture) combination.
// Generated by scripts/generate-interior-placeholders.mjs into
// public/static-assets/interior-placeholders/<room>__<flooring>__<cabinets>__<furniture>.png
// (PNG, not SVG -- photo-sphere-viewer/three.js can't use an SVG <img> as a
// WebGL texture source in this stack; PNG works fine.)
function interiorPlaceholderUrl(room: Room, flooringId: string, cabinetsId: string, furnitureId: string) {
  const key = `${room}__${flooringId}__${cabinetsId}__${furnitureId}`;
  return `${import.meta.env.BASE_URL}static-assets/interior-placeholders/${key}.png`;
}
...
// Furniture categories that correspond to one of the 4 room tabs (Outdoors doesn't)
const roomForCategory: Partial<Record<FurnitureCategory, Room>> = {
  'Living Room': 'living',
  'Bedroom': 'bedroom',
  'Bathroom': 'bathroom',
};
...
export function InteriorStudio({ onSwitchView, onConfirm, onNext }: InteriorStudioProps) {
  ...
  const selectedPackage = furniturePackages.find(p => p.id === selectedFurniture) ?? null;

  const furniturePrice = selectedPackage?.price || 0;

  // The furniture package only counts toward the rendered image when it's
  // actually for the room being viewed (e.g. a Bedroom package selected while
  // still viewing Living has no matching placeholder) -- otherwise treat as
  // unfurnished for image-lookup purposes, same as before this change.
  const furnitureIdForImage =
    selectedPackage && roomForCategory[selectedPackage.category as FurnitureCategory] === focusedRoom
      ? selectedPackage.id
      : 'unfurnished';

  const displayRoomImage = interiorPlaceholderUrl(
    focusedRoom,
    selections.flooring,
    selections.cabinets,
    furnitureIdForImage
                    ...
                    <div className="grid grid-cols-1 gap-4">
                       {filteredFurniture.map((pkg) => (
                          <button
                             key={pkg.id}
                             onClick={() => {
                               setSelectedFurniture(pkg.id);
                               const room = roomForCategory[pkg.category as FurnitureCategory];
                               if (room) setFocusedRoom(room);
                             }}
