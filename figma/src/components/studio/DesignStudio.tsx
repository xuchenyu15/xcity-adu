...
export function DesignStudio({ onSignOut }: DesignStudioProps) {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) setLookup(JSON.parse(raw));
    } catch {
    }
  }, []);
      ...
      case 'design':
        ...
        return (
          <div className="flex-1 flex overflow-hidden bg-white min-h-0">
            ...
            <div className="flex-1 relative bg-slate-50 border-r border-slate-100 min-h-0">
              <SiteVisualizer 
            ...
            <div className="w-[420px] shrink-0 flex flex-col relative h-full bg-white border-l border-slate-200 min-h-0">
              <div className="flex-1 overflow-y-auto no-scrollbar px-8 pt-10 pb-72 min-h-0">
  ...
  const projectTabs = [
    { id: 'site', label: t('studio.projectTabs.site'), status: completedTabs.has('site') ? 'completed' : 'empty' },
    { id: 'design', label: t('studio.projectTabs.design'), status: completedTabs.has('design') ? 'completed' : completedTabs.has('site') ? 'empty' : 'disabled' },
    { id: 'value', label: t('studio.projectTabs.value'), status: completedTabs.has('value') ? 'completed' : completedTabs.has('design') ? 'empty' : 'disabled' },
    { id: 'timeline', label: t('studio.projectTabs.timeline'), status: completedTabs.has('value') ? 'empty' : 'disabled' },
  ];
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 font-sans">
       ...
       {/* Main Content Area */}
       <div className="overflow-hidden relative flex flex-col h-[calc(100vh-72px)] mt-[72px] min-h-0">
           {renderContent()}
       </div>
    </div>
  );
