...
export function FreeBuildSection(props: FreeBuildSectionProps) {
  ...
  const buyback = useMemo(() => computeBuybackSchedule(fb.totalCapital), [fb.totalCapital]);
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="p-6 rounded-xl bg-[#2B7FFF]/5 border border-[#2B7FFF]/15">
          ...
          <p className="text-[13px] text-slate-400 mt-2">
            Your 30% share (Year 1–3) · Year 4–5 {formatCurrency(fb.ownerMonthlyY4)}/mo · Year 6+ {formatCurrency(fb.ownerMonthlyY6)}/mo
          </p>
        </div>
      </div>
      ...
      <div className="mt-12">
        ...
        <ExitBuybackModule
          ...
          shareNote={'tiered revenue share (you 30% → 60%)'}
        />
      </div>
    </div>
  );
