...
public sealed class NewJerseyOfficialLookupProvider : ILookupProvider
{
    private readonly IHttpClientFactory _httpFactory;
    private readonly OsmBuildingsService _osmBuildings;
    private readonly Microsoft.Extensions.Logging.ILogger<NewJerseyOfficialLookupProvider> _logger;

    public NewJerseyOfficialLookupProvider(IHttpClientFactory httpFactory, OsmBuildingsService osmBuildings, Microsoft.Extensions.Logging.ILogger<NewJerseyOfficialLookupProvider> logger)
    {
    ...
    public string ProviderName => "nj-official";
    public int Priority => 80;

    public bool CanHandle(string address, string state)
    {
        return string.Equals(state?.Trim(), "NJ", StringComparison.OrdinalIgnoreCase);
    }
    ...
    public async Task<LookupProviderResult?> LookupAsync(string address, string state, CancellationToken cancellationToken)
    {
        ...
        _logger.LogInformation("新泽西官方地理编码请求 url={Url}", geocodeUrl);
        using var geoResp = await SendGetWithRetryAsync(http, geocodeUrl, cancellationToken);
        _logger.LogInformation("新泽西官方地理编码响应 status={StatusCode}", (int)geoResp.StatusCode);
        if (!geoResp.IsSuccessStatusCode) return null;

        var geoText = await geoResp.Content.ReadAsStringAsync(cancellationToken);
        _logger.LogInformation("新泽西官方地理编码返回 len={Len} body={Body}", geoText.Length, Truncate(geoText, 8000));
        ...
        var parcelUrl = string.Format(
            ...
            "https://maps.nj.gov/arcgis/rest/services/Framework/Cadastral/MapServer/0/query?f=geojson&geometryType=esriGeometryPoint&inSR=4326&geometry={0},{1}&spatialRel=esriSpatialRelIntersects&outFields=*&returnGeometry=true&outSR=4326&resultRecordCount=1",
            lon,
            lat);
        ...
        try
        {
            pid = rawProps["PAMS_PIN"]?.GetValue<string>()
                  ?? rawProps["pams_pin"]?.GetValue<string>()
                  ?? rawProps["GIS_PIN"]?.GetValue<string>()
                  ?? rawProps["gis_pin"]?.GetValue<string>()
                  ?? rawProps["PIN_NODUP"]?.GetValue<string>()
                  ?? rawProps["pin_nodup"]?.GetValue<string>()
                  ?? "";
            if (string.IsNullOrWhiteSpace(pid))
            {
                var block = rawProps["PCLBLOCK"]?.GetValue<string>() ?? rawProps["pclblock"]?.GetValue<string>() ?? "";
                var lot = rawProps["PCLLOT"]?.GetValue<string>() ?? rawProps["pcllot"]?.GetValue<string>() ?? "";
                if (!string.IsNullOrWhiteSpace(block) || !string.IsNullOrWhiteSpace(lot))
                    pid = $"{block}:{lot}";
            }
        }
        catch
        ...
        if (string.IsNullOrWhiteSpace(city))
        {
            try
            {
                city = rawProps["MUN_NAME"]?.GetValue<string>() ?? city;
            }
            catch
            {
            }
        ...
        {
            ["ll_uuid"] = string.IsNullOrWhiteSpace(pid) ? $"nj:{lon:F6},{lat:F6}" : $"nj:{pid}",
            ["parcelnumb"] = pid,
            ["apn"] = pid,
            ["scity"] = city,
            ["sstate"] = "NJ",
