  ...
  const renderMarketingNav = () => (
    <nav className="flex items-center gap-1 p-1 rounded-full transition-all border shadow-sm backdrop-blur-md bg-white/10 border-white/5">
      {['models', 'how-it-works', 'earn', 'about'].map((tab) => {
         const label = tab === 'models' ? 'Models' : 
                      tab === 'how-it-works' ? 'How It Works' : 
                      tab === 'earn' ? 'How You Earn' : 
                      'About XBuild';
    ...
    <header className={`h-[72px] w-full grid grid-cols-3 items-center px-6 md:px-8 z-50 transition-colors fixed top-0 left-0 ${headerBg} ${className.replace('fixed top-0 left-0 w-full', '')}`}>
        ...
        <div className="flex items-center gap-4 justify-self-end">
             ...
             <div className={`flex rounded-lg p-1 gap-1 border transition-colors ${isDark ? 'bg-slate-900/60 border-slate-700' : 'bg-slate-100 border-slate-200'}`}>
                 <button onClick={() => onLanguageChange?.('en')} className={`px-2 py-1 text-[10px] font-bold rounded transition-all ${language === 'en' ? (isDark ? 'bg-blue-600 text-white' : 'bg-white text-slate-900 shadow-sm') : (isDark ? 'text-slate-400 hover:text-white' : 'text-slate-500 hover:text-slate-900')}`}>En</button>
                 <button onClick={() => onLanguageChange?.('zh')} className={`px-2 py-1 text-[10px] font-bold rounded transition-all ${language === 'zh' ? (isDark ? 'bg-blue-600 text-white' : 'bg-white text-slate-900 shadow-sm') : (isDark ? 'text-slate-400 hover:text-white' : 'text-slate-500 hover:text-slate-900')}`}>中</button>
             </div>
             
             <div className={`h-6 w-px mx-1 ${isDark ? 'bg-slate-700' : 'bg-slate-200'}`} />

             {!isAuthenticated ? (
                 <button onClick={onSignIn} className={`px-5 py-2.5 rounded-xl font-bold text-sm transition-all shadow-lg ${isDark ? 'bg-white text-slate-900 hover:bg-slate-200' : 'bg-slate-900 text-white hover:bg-slate-800'}`}>Sign In</button>
             ) : (
