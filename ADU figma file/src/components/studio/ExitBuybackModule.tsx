...
const BASE_SHARE = 1450;
const RENT_GROWTH = 0.03;

function computeCumulativeIncome(): number[] {
  const data: number[] = [0];
  let cumulative = 0;
  for (let yr = 1; yr <= MAX_YEAR; yr++) {
    const monthlyShare = BASE_SHARE * Math.pow(1 + RENT_GROWTH, yr - 1);
...
function computeEstRent(): number[] {
  const rents: number[] = [];
  for (let yr = 0; yr <= MAX_YEAR; yr++) {
    rents.push(Math.round(2900 * Math.pow(1 + RENT_GROWTH, yr)));
  }
  return rents;
}

const CUMULATIVE_DATA = computeCumulativeIncome();
const EST_RENT = computeEstRent();

const GUIDANCE_BLOCKS = [
  {
    icon: Clock,
    label: 'Early Exit (1–3 yrs)',
    focus: 'Liquidity',
    tradeoff: 'Lower total return',
  },
  {
    icon: Scale,
    label: 'Mid Hold (4–6 yrs)',
    focus: 'Balanced growth',
    tradeoff: 'Moderate capital lock-in',
  },
  {
    icon: TrendingUp,
    label: 'Long Hold (7–10 yrs)',
    focus: 'Maximum cumulative income',
    tradeoff: 'Longer liquidity horizon',
  },
];
...
export function ExitBuybackModule({
  ...
  // Panel data (only used when year is selected)
  ...
  const incomeEarned = year !== null ? CUMULATIVE_DATA[year] : 0;
  const estRent = year !== null ? EST_RENT[year] : 0;
  ...
  const maxVal = useMemo(() => {
    const step = 50000;
    return Math.ceil(CUMULATIVE_DATA[MAX_YEAR] / step) * step;
  }, []);
  ...
  const points = useMemo(() => {
    return CUMULATIVE_DATA.map((val, i) => ({
  ...
  return (
    <div className="rounded-2xl bg-white border border-slate-200 shadow-sm mb-6">
      {/* Header */}
      <div className="px-8 pt-8 pb-4">
        <h3 className="text-[20px] font-semibold tracking-tight text-slate-900 mb-2">Asset Growth & Exit Value</h3>
        <p className="text-[14px] font-medium text-slate-500">
          Cumulative Net Income (USD) · 3% annual growth · 50/50 revenue share
        </p>
      </div>
      ...
      <div className="px-8 pb-6 flex flex-col lg:flex-row gap-6">
        ...
        >
          ...
          >
            ...
            {yGridValues.map((val) => {
              ...
              return (
                <g key={val}>
                  ...
                  <text x={PAD.left - 10} y={gy + 4} textAnchor="end" fill="#94a3b8" fontSize="12" fontWeight="400" fontFamily="system-ui, sans-serif">
                    {fmtK(val)}
                  </text>
                </g>
              );
            ...
            {/* Hover highlight — light vertical line */}
            {hoverYear !== null && hoverYear !== year && (() => {
              const hp = points[hoverYear];
              return (
                <line
                  x1={hp.x} y1={PAD.top} x2={hp.x} y2={PAD.top + chartH}
                  stroke={BLUE} strokeWidth="1" strokeDasharray="3 3" opacity="0.15"
                />
              );
            })()}

            {/* Selected year — vertical dashed indicator (only when selected) */}
            {selectedPoint && (
              <line
                x1={selectedPoint.x}
                y1={PAD.top}
            ...
            {points.map((p, i) => {
              ...
              // Default: uniform small neutral dots
              return (
                <circle
                  key={`dot-${i}`}
                  cx={p.x}
                  cy={p.y}
                  r={2.5}
                  fill={hasSelection && year !== null && i < year ? BLUE : '#cbd5e1'}
                  stroke="white"
                  strokeWidth="1"
            ...
            {hoverYear !== null && hoverYear !== year && hoverPoint && (() => {
              ...
              const pillY = hoverPoint.y - 30;
              const clampedY = Math.max(PAD.top, pillY);
              return (
                <g>
                  <rect
                    x={pillX}
        ...
        <div className="lg:w-[240px] shrink-0 flex flex-col">
          {!hasSelection ? (
            /* ─── DEFAULT STATE: Decision Guidance ─── */
            <div className="flex flex-col h-full">
              ...
              <p className="text-[12.5px] text-slate-500 leading-relaxed mb-6">
                Exiting earlier lowers risk and shortens capital lock-in.
                Holding longer increases cumulative income but delays liquidity.
              </p>
                ...
                {GUIDANCE_BLOCKS.map((block, i) => {
                  ...
                  return (
                    <div key={i} className="rounded-xl border border-slate-100 bg-slate-50/60 px-4 py-3">
                      ...
                      <div className="flex items-center gap-3 text-[11px] text-slate-400 pl-[22px]">
                        <span>Focus: <span className="text-slate-500">{block.focus}</span></span>
                      </div>
                      <div className="flex items-center gap-3 text-[11px] text-slate-400 pl-[22px] mt-0.5">
                        <span>Trade-off: <span className="text-slate-500">{block.tradeoff}</span></span>
                      </div>
                    </div>
                  );
                })}
            ...
            <div className="flex flex-col h-full">
              ...
              <div className="space-y-4 mb-6">
                ...
                <div>
                  <SubsectionLabel>Cumulative Income</SubsectionLabel>
                  <div className="text-[14px] text-slate-700 mt-0.5">
                    {year === 0 ? '—' : fmtCurrency(incomeEarned)}
                  </div>
                </div>

                <div>
                  <SubsectionLabel>Est. Rent</SubsectionLabel>
              ...
              {lockState === 'unlocked' && (
                <div>
                  <button
                    onClick={onLock}
                    className="w-full h-10 bg-[#2B7FFF] text-white rounded-xl text-[15px] font-medium flex items-center justify-center gap-1.5 hover:bg-blue-600 transition-colors active:scale-[0.98] cursor-pointer"
                  >
                    Lock Year {year} Buyback
              ...
              {lockState === 'final' && (
                <div>
                  <button
                    disabled
                    className="w-full h-10 bg-slate-100 text-slate-400 rounded-xl text-[15px] font-medium flex items-center justify-center gap-1.5 cursor-not-allowed"
                  >
                    <Lock className="w-3.5 h-3.5" />
                    Locked · Final
                  </button>
                  <p className="text-[11px] text-slate-400 mt-2.5">
                    Multiplier secured at {multiplier.toFixed(2)}×
                  </p>
                </div>
              )}
